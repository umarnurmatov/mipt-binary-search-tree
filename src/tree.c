#include "tree.h"

#include <cstdlib>
#include <string.h>
#include <time.h>

#include "logutils.h"
#include "memutils.h"
#include "ioutils.h"

#ifdef _DEBUG

#define TREE_DUMP_(tree, err) \
    tree_dump_(tree, err, NULL, __FILE__, __LINE__, __func__); 

#define TREE_DUMP_MSG_(tree, err, msg) \
    tree_dump_(tree, err, msg, __FILE__, __LINE__, __func__); 

#define TREE_ASSERT_OK_(dllist)                      \
    {                                                \
        tree_err_t err = tree_verify_(tree);         \
        if(err != TREE_ERR_NONE) {                   \
            TREE_DUMP_(tree, err);                   \
            utils_assert(err == TREE_ERR_NONE);      \
        }                                            \
    }

#define TREE_VERIFY_OR_RETURN_(tree, err)       \
    if(err != TREE_ERR_NONE) {                  \
        TREE_DUMP_(tree, err);                  \
        return err;                             \
    }

#else // _DEBUG

// FIXME

#endif // _DEBUG

tree_err_t tree_allocate_new_(tree_node_t** ptr);

void tree_dump_(tree_t* tree, tree_err_t err, const char* msg, const char* file, int line, const char* funcname);

char* tree_dump_graphviz_(tree_t* tree);

int tree_dump_node_graphviz_(FILE* file, tree_node_t* node, int rank);

#ifdef _DEBUG

tree_err_t tree_verify_(tree_t* tree);

void tree_node_dtor_(tree_node_t* node);

#endif // _DEBUG

tree_err_t tree_insert(tree_t* tree, tree_data_t data)
{
    TREE_ASSERT_OK_(tree);

    tree_err_t err = TREE_ERR_NONE;

    tree_node_t* parent = tree->root;
    tree_node_t* child = tree->root;

    while(child != NULL) {
        parent = child;

        if(data <= parent->data)
            child = parent->left;
        else
            child = parent->right;
    }
    
    tree_node_t* new_elem;
    err = tree_allocate_new_(&new_elem);
    TREE_VERIFY_OR_RETURN_(tree, err);

    new_elem->data = data;
    
    if(!parent) {
        tree->root = new_elem;
    }
    else {
        if(data <= parent->data)
            parent->left = new_elem;
        else
            parent->right = new_elem;
    }

    ++tree->size;

    TREE_DUMP_(tree, err);

    return TREE_ERR_NONE;
}


const char* tree_strerr(tree_err_t err)
{
    switch(err) {
        case TREE_ERR_NONE:
            return "none";
        case TREE_NULLPTR:
            return "passed a nullptr";
        case TREE_ALLOC_FAIL:
            return "memory allocation failed";
        default:
            return "unknown";
    }
}

void tree_dtor(tree_t* tree)
{
    utils_assert(tree);

    tree_node_dtor_(tree->root); 
}

tree_err_t tree_allocate_new_(tree_node_t** ptr)
{
    utils_assert(ptr);

    tree_node_t* ptr_tmp = (tree_node_t*)calloc(1, sizeof(ptr_tmp[0]));

    if(!ptr_tmp)
        return TREE_ALLOC_FAIL;

    *ptr = ptr_tmp;

    return TREE_ERR_NONE;
}

void tree_node_dtor_(tree_node_t* node)
{
    utils_assert(node);

    if(node->left)
        tree_node_dtor_(node->left);
    if(node->right)
        tree_node_dtor_(node->right);

    NFREE(node);
}

#ifdef _DEBUG

#define GRAPHVIZ_FNAME_ "graphviz"
#define GRAPHVIZ_CMD_LEN_ 100

#define CLR_RED_LIGHT_   "\"#FFB0B0\""
#define CLR_GREEN_LIGHT_ "\"#B0FFB0\""
#define CLR_BLUE_LIGHT_  "\"#B0B0FF\""

#define CLR_RED_BOLD_    "\"#FF0000\""
#define CLR_GREEN_BOLD_  "\"#03c03c\""
#define CLR_BLUE_BOLD_   "\"#0000FF\""

void tree_dump_(tree_t* tree, tree_err_t err, const char* msg, const char* filename, int line, const char* funcname)
{
    utils_log_fprintf(
        "<style>"
        "table {"
          "border-collapse: collapse;"
          "border: 1px solid;"
          "font-size: 0.9em;"
        "}"
        "th,"
        "td {"
          "border: 1px solid rgb(160 160 160);"
          "padding: 8px 10px;"
        "}"
        "</style>\n"
    );

    utils_log_fprintf("<pre>\n"); 

    time_t cur_time = time(NULL);
    struct tm* iso_time = localtime(&cur_time);
    char time_buff[100];
    strftime(time_buff, sizeof(time_buff), "%F %T", iso_time);

    if(err != TREE_ERR_NONE) {
        utils_log_fprintf("<h3 style=\"color:red;\">[ERROR] [%s] from %s:%d: %s() </h3>", time_buff, filename, line, funcname);
        utils_log_fprintf("<h4><font color=\"red\">err: %s </font></h4>", tree_strerr(err));
    }
    else
        utils_log_fprintf("<h3>[DEBUG] [%s] from %s:%d: %s() </h3>\n", time_buff, filename, line, funcname);

    if(msg)
        utils_log_fprintf("what: %s\n", msg);

    char* img_pref = tree_dump_graphviz_(tree);

    utils_log_fprintf(
        "\n<img src=" IMG_DIR "/%s.svg width=50%%\n", 
        strrchr(img_pref, '/') + 1
    );

    utils_log_fprintf("</pre>\n"); 

    utils_log_fprintf("<hr color=\"black\" />\n");

    NFREE(img_pref);
}

char* tree_dump_graphviz_(tree_t* tree)
{

    FILE* file = open_file(LOG_DIR "/" GRAPHVIZ_FNAME_ ".txt", "w");
    if(!file)
        exit(EXIT_FAILURE);

    fprintf(file, "digraph {\n rankdir=TB;\n"); 
    fprintf(file, "nodesep=0.9;\nranksep=0.75;\n");

    tree_dump_node_graphviz_(file, tree->root, 1);

    fprintf(file, "};");

    fclose(file);
    
    create_dir(LOG_DIR "/" IMG_DIR);
    char* img_tmpnam = tempnam(LOG_DIR "/" IMG_DIR, "img-");
    utils_assert(img_tmpnam);

    static char strbuf[GRAPHVIZ_CMD_LEN_]= "";

    sprintf(
        strbuf, 
        "dot -T svg -o %s.svg " LOG_DIR "/" GRAPHVIZ_FNAME_ ".txt", 
        img_tmpnam
    );

    system(strbuf);

    return img_tmpnam;
}

int tree_dump_node_graphviz_(FILE* file, tree_node_t* node, int rank)
{
    utils_assert(file);
    utils_assert(node);

    static int node_cnt = 0;

    if(rank == 1)
        node_cnt = 0;

    int node_cnt_left = 0, node_cnt_right = 0;
    
    if(node->left)
        node_cnt_left = tree_dump_node_graphviz_(file, node->left, rank + 1);
    if(node->right)
        node_cnt_right = tree_dump_node_graphviz_(file, node->right, rank + 1);
    
    fprintf(
        file, 
        "node_%d["
        "shape=record,"
        "label=\" { data: %d | { %p | %p } } \","
        "rank=%d"
        "];\n",
        node_cnt,
        node->data,
        node->left,
        node->right,
        rank
    );

    if(node->left)
        fprintf(
            file,
            "node_%d -> node_%d;\n",
            node_cnt, 
            node_cnt_left
        );

    if(node->right)
        fprintf(
            file,
            "node_%d -> node_%d;\n",
            node_cnt, 
            node_cnt_right
        );

    return node_cnt++;
}

tree_err_t tree_verify_(tree_t* tree)
{
    if(!tree)
        return TREE_NULLPTR;

    return TREE_ERR_NONE;
}


#endif // _DEBUG
